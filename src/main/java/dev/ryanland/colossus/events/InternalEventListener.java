...
public class InternalEventListener extends ListenerAdapter {
    ...
    public void onMessageReceived(@NotNull MessageReceivedEvent event) {
        if (!event.getAuthor().isBot()) {
            if (Config.getBoolean("message_commands.enabled") == null) return;
            if (!Config.getBoolean("message_commands.enabled") || Config.getString("message_commands.prefix") == null) return;
